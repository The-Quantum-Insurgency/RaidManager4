...
module.exports = {
  name: "addlog",
  description:
    "Adds an event to the total amount of events a player has attended.",
  options: [
    ...
    new SlashCommandNumberOption()
      .setName("amount")
      .setDescription("(OPTIONAL) The amount of events to add to the user.")
      .setRequired(false),
  ],
  ...
  execute: async function (RaidManager, Interaction) {
    ...
    const InteractionEmbed = new MessageEmbed()
      ...
      .setDescription(`Adding ${Amount} event log(s) to <@${APIUser.id}>.`)
      .setColor("ORANGE");
    ...
    if (User) {
      ...
      if (EventsSaved) {
        InteractionEmbed.setDescription(
          `Successfully logged ${Amount} event(s) to <@${APIUser.id}>.`
        );
